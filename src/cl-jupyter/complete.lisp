...
(defun complete-package (match-set partial-name start end &key include-marker)
  (dolist (name (append (mapcar #'package-name (list-all-packages))
                        ...
                        #+(and package-local-nicknames (or abcl clasp ecl))
                          (mapcar #'car (ext:package-local-nicknames *package*))
                        #+(and package-local-nicknames allegro)
                          (mapcar #'car (excl:package-local-nicknames *package*))
                        #+(and package-local-nicknames ccl)
                          (mapcar #'car (ccl:package-local-nicknames *package*))
                        #+(and package-local-nicknames lispworks)
                          (mapcar #'car (hcl:package-local-nicknames *package*))
                        #+(and package-local-nicknames sbcl)
                          (mapcar #'car (sb-ext:package-local-nicknames *package*))))
